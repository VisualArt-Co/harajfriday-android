...
class HarajDetailsFragment : Fragment() {

    companion object {
        fun newInstance(product: DataItem): HarajDetailsFragment {
            val args = Bundle()
            args.putSerializable(PRODUCT, product)
            val harajDetailsFragment = HarajDetailsFragment()
            harajDetailsFragment.product = product
            harajDetailsFragment.arguments = args
            return harajDetailsFragment
        }

        fun newInstance(harajId: String): HarajDetailsFragment {
            val args = Bundle()
    ...
    private fun updateUI() {
        if (product != null) {
            ...
            shareImageView.setOnClickListener {
                val shareBody = String.format(Locale.US, "%s%s", getBaseURL(), product?.id)
